            ...
            <div :class="open ? 'block': 'hidden'" class="w-full flex-grow sm:flex sm:items-center sm:w-auto">
                <div :class="open ? 'block': 'hidden'" class="w-full flex-grow sm:flex sm:items-center sm:w-auto">
                    <div class="w-full container mx-auto flex flex-col sm:flex-row items-center justify-between text-sm font-bold uppercase mt-0 px-6 py-2">
                        ...
                        <div class="flex mt-4 sm:mt-0"> <!-- Wrap the auth and dropdown in a flex container -->
                            @auth
                            <div x-data="{ isOpen: false }" class="relative inline-block text-left">
                                <div>
                                    <button @click="isOpen = !isOpen" type="button" class="bg-black rounded text-white flex py-2 px-4 mx-2" id="menu-button" :aria-expanded="isOpen" aria-haspopup="true">
                                        {{ Auth::user()->name }}
                                ...
                                <div x-show="isOpen" @click.away="isOpen = false" @keydown.escape="isOpen = false" class="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" :aria-labelledby="isOpen ? 'menu-button' : null" tabindex="-1">
                                    <div class="py-1" role="none">
                                        <!-- Active: "bg-gray-100 text-gray-900", Not Active: "text-gray-700" -->
                                        <a href="{{ route('profile.edit')}}" class="text-gray-700 block px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-0">{{ __('Profile') }}</a>
                
                                        <form method="POST" action="{{ route('logout') }}" role="none">
                                            @csrf
                                            <button type="submit" class="text-gray-700 block w-full px-4 py-2 text-left text-sm" role="menuitem" tabindex="-1" id="menu-item-3">{{ __('Log Out') }} </button>
                                        </form>
                                    </div>
                                </div>
                            </div>
                            @else
                            <a href="{{ route('login') }}" class="bg-blue-800 text-white rounded py-2 px-4 mx-2">Login</a>
                            <a href="{{ ('register') }}" class="bg-blue-800 text-white rounded py-2 px-4 mx-2">Register</a>
                            @endauth
                        </div>
                    </div>
                </div>
